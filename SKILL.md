...
description_zh: "全球首个开源喜剧 AI + 视频剪辑 + 海报生成工具。创作脱口秀/小品/漫才/剧本等喜剧内容，或通过自然语言脚本驱动 FFmpeg 进行视频剪辑，或生成脱口秀/演出/金句海报及大麦/猫眼/秀动等平台标准规格物料。支持动态生成编剧提示词（Yes, and / 预期违背 / callback），内置过渡词禁令规范。"
...
**在读取全部知识后、正式创作前，动态生成角色提示词。**

根据以下信息构建：
- **体裁**：脱口秀 / 漫才 / Sketch / 日式短剧 等
- **风格**：Step 2A 风格选择步骤确定的调性（如"自嘲式"、"荒诞系"、"解构式"等）

生成的提示词格式如下（**直接用于引导创作过程，不得跳过**）：

> **动态编剧提示词**
> "你是一位擅长'预期违背'和'callback'的 **[体裁]** 编剧。
> 你的风格类似于 **[风格]**。
> 请按以下步骤工作：
> 根据【当前话题：**<用户输入的话题>**】，先列出3个常见刻板印象或生活观察。
> 对每个观察，用'是的，而且…'（Yes, and）方法推演一个荒谬但逻辑自洽的结论。
> 选择最有趣的一条，用'铺垫-笑点-再解读'结构写段落的初稿。"

将此提示词作为**创作启动器**，让创作从刻板印象挖掘和荒谬推演开始，而非直接套模板。
...
> **说明**：这些词会让喜剧显得犹疑、妥协、缺乏自信。好的喜剧段子应当像拳头一样直接打到观众脸上，不需要任何铺垫软化。
